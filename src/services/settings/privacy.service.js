...
export const updatePrivacySettings = async (userId, rawData) => {
    ...
    if (!updated) {
        // BUG FIXED: error message was the success log message
        throw new AppError('Privacy settings not found.', 404);
    }
...
/**
 * requestDataDownload — GDPR data export
 *
 * BUG FIXED: original condition was `if (!existing?.dataDownloadRequestedAt)`
 * which means it would enter the rate-limit block when there IS NO prior request.
 * Logic should be: if a request WAS made recently → block.
 */
export const requestDataDownload = async (userId) => {
    const existing = await privacyRepo.findByUserId(userId);

    // If a prior request exists, enforce 24h cooldown
    if (existing?.dataDownloadRequestedAt) {
        ...
        if (hoursSince < 24) {
            throw new AppError(
                'Data download already requested. Please wait 24 hours.',
                429,
                'TOO_MANY_REQUESTS'
            );
        }
 ...
 *
 * BUG FIXED: original condition was `if (!existing?.deleteAccountRequestedAt)`
 * which means it threw when there was NO prior request — exact opposite of what's needed.
 * Should throw when a deletion IS already scheduled.
 */
export const requestAccountDeletion = async (userId) => {
    ...
    logger.warn(
        `Account deletion requested by user ${userId} — scheduled: ${dto.deleteAccountScheduledAt}`
    );
