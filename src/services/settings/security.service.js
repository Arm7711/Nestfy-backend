...
import * as sessionSvc   from '../../services/auth/session.service.js';
 ...
 *
 * BUGS FIXED:
 * 1. `isSame` check compared newPassword against current hash — correct direction
 *    but was used to block (should block if same). Original was fine here.
 * 2. `dto.newPassword !== dto.confirmPassword` check is correct.
 * 3. BUG: `bcrypt.hash(userId, 12)` — was hashing the userId string instead of
 *    the new password. Should be `bcrypt.hash(dto.newPassword, 12)`.
 */
...
export const toggleTwoFactor = async (userId, rawData) => {
    ...
    const updateData = {
        ...
        twoFactorMethod:  dto.enabled ? dto.method : null,
...
export const revokeSession = async (userId, sessionId) => {
    ...
    if (!owns) throw new AppError('Session not found or access denied.', 403, 'FORBIDDEN');
